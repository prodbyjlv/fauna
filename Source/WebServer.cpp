...
juce::String HTTPServer::getHTMLPage()
{
    ...
    html+=".unlock-overlay{position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);display:flex;justify-content:center;align-items:center;z-index:9999}";
    html+=".unlock-btn{padding:25px 50px;font-size:1.5em;background:#00ff88;color:#1a1a2e;border:none;border-radius:15px;cursor:pointer;font-weight:bold;box-shadow:0 4px 15px rgba(0,255,136,0.3)}";
    html+=".unlock-btn:active{transform:scale(0.98)}";
    html+=".unlock-overlay.hidden{display:none}";
    html+="</style></head><body>";
    html+="<div class=\"unlock-overlay\" id=\"unlockOverlay\">";
    html+="<button class=\"unlock-btn\" id=\"unlockBtn\">TAP TO CONNECT</button>";
    html+="</div>";
    ...
    html+="</div>";
    html+="<button class=\"start-btn\" id=\"startBtn\">START AUDIO</button>";
    ...
    html+="var isMuted=false,ws=null,audioCtx=null,initReceived=false,audioBufferCount=0;";
    html+="var nextPlayTime=0,SAMPLE_RATE=44100;";

    html+="document.getElementById('unlockBtn').addEventListener('click',async function(){";
    html+="if(!audioCtx){audioCtx=new(window.AudioContext||window.webkitAudioContext)();}";
    html+="if(audioCtx.state==='suspended'){await audioCtx.resume();console.log('AudioContext state:',audioCtx.state);}";
    html+="document.getElementById('unlockOverlay').classList.add('hidden');";
    ...
    html+="ws.onopen=function(){document.getElementById('status').textContent='Connected';document.getElementById('audioStatus').textContent='Waiting for DAW...';};";
    html+="ws.onclose=function(e){";
    html+="document.getElementById('audioStatus').textContent='Disconnected';";
    html+="document.getElementById('audioStatus').style.color='#ff4444';";
    html+="initReceived=false;audioBufferCount=0;";
    html+="setTimeout(function(){if(audioCtx&&audioCtx.state==='suspended'){audioCtx.resume().then(connectWS);}else{connectWS();}},3000);";
    html+="};";
